...
use crate::models::{
    ...
    CollectionSummary, ImageMeta, ItemKind, ListQuery, NewItem, PlatformKind, Settings, SourceApp, StoredFile,
    SyncStatus,
};
...
"#,
    ...
    pub fn collections(&self) -> Result<Vec<CollectionSummary>> {
        ...
        let rows = statement.query_map([], |row| Ok(CollectionSummary { name: row.get(0)?, item_count: row.get(1)? }))?;
        rows.collect::<std::result::Result<Vec<_>, _>>().map_err(Error::from)
    }

    pub fn create_collection(&self, name: &str) -> Result<()> {
        let name = normalize_collection_name(name)?;
        self.conn.lock().execute("INSERT OR IGNORE INTO collections (name) VALUES (?1)", params![name])?;
        Ok(())
    }
    ...
    pub fn delete_collection(&self, name: &str) -> Result<()> {
        ...
        let rows = statement.query_map(params![name], |row| Ok((row.get::<_, i64>(0)?, row.get::<_, String>(1)?)))?;
        ...
        for (id, tags) in affected {
            ...
            let remaining: Vec<String> = tags.into_iter().filter(|tag| !tag.eq_ignore_ascii_case(name.as_str())).collect();
            let remaining_json = serde_json::to_string(&remaining)
                .map_err(|error| Error::Other(error.to_string()))?;
            conn.execute("UPDATE items SET tags = ?2 WHERE id = ?1", params![id, remaining_json])?;
        }
        Ok(())
    }
    ...
    pub fn set_tags(&self, id: i64, tags: &[String]) -> Result<ClipItem> {
        ...
        for tag in &normalized {
            conn.execute("INSERT OR IGNORE INTO collections (name) VALUES (?1)", params![tag])?;
        }
...
fn normalize_collection_name(name: &str) -> Result<String> {
    let normalized = name.trim().trim_start_matches('#').chars().take(40).collect::<String>();
